...
class ImageNetHelper:
        ...
        def buildClassLabels(self):
                # to load the contents of the file that maps the WordNet IDs
                # to integers, then initialize the labels mappings dictionary
                rows = open(self.config.WORD_IDS).read().strip().split("\n")
                labelMappings = {}

                # loop over the labels
                for row in rows:
                        # split the two into the WordNet ID, label integer, and 
        ...
        def buildValidationSet(self):
                ...
                # load the contents of the filw that contains the *actual*
